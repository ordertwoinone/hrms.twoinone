...
export function EmiratesIdFormDialog({
  open,
  onOpenChange,
  options,
  record,
  ...
  const form = useForm<EmiratesIdFormInput>({
    resolver: zodResolver(emiratesIdFormSchema),
    defaultValues: EMPTY,
    values: record
      ? {
          employee_id: record.employeeId,
          eid_number: record.eidNumber,
  ...
  async function onSubmit(values: EmiratesIdFormInput) {
    const fd = new FormData();
    if (isEdit) fd.set("id", record!.id);
    fd.set("employee_id", values.employee_id);
    fd.set("eid_number", values.eid_number);
    ...
    const result = isEdit
      ? await updateEmiratesIdAction(fd)
      : await createEmiratesIdAction(fd);
    if (!result.success) {
      toast.error(result.error);
      return;
    }
    toast.success(isEdit ? "Emirates ID updated." : "Emirates ID added.");
    if (!isEdit) reset();
    onOpenChange(false);
    router.refresh();
  }
  ...
  const text = (
    name: keyof EmiratesIdFormInput,
    label: string,
    type = "text",
  ) => (
    <FormField
  ...
  return (
    ...
    >
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEdit ? "Edit Emirates ID" : "Add Emirates ID"}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? "Update this Emirates ID record and its document."
              : "Register an Emirates ID against an employee."}
          </DialogDescription>
        </DialogHeader>
