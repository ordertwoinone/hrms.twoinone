...
import type { ExpiryLevel, PolicyStatus } from "./types";

/** Common UAE health-plan tiers. Free-text is still allowed on the form. */
...
/**
 * Classify a policy's expiry urgency. Cancelled/renewed records are never
 * flagged. Pure — safe on both server and client.
 */
export function expiryLevel(expiryISO: string, status: PolicyStatus): ExpiryLevel {
